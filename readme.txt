...
Tags:网站加速, 缓存, redis缓存, Memcache缓存, 页面加速, 静态化, 果果加速
...
支持文件、Redis、Memcache缓存加速，让页面浏览更快！

== Description ==

支持将网站前台页面内容缓存到文件、Redis、Memcache里，以此来加速网站页面的访问速度。
...
1、支持设置缓存方式，是使用文件缓存还是Redis缓存或者Memcache缓存。

2、可以设置页面内容的缓存有效期。

3、友好的错误提示文字，让您轻松设置Redis缓存或者Memcache缓存。
...
= 如何使用Memcache缓存？ =
需要安装PHP Memcache扩展，以及Memcache软件。宝塔面板用户可以直接从软件商店安装。
